...
handle_endpoint(function () use ($library) {
	...
	$fileIds = isset($payload['file_ids']) ? $payload['file_ids'] : null;
	if ($fileIds === null) {
		throw new InvalidArgumentException('file_ids is required');
	}

	$result = $library->attachFilesToBook($bookId, $fileIds);
	json_success($result);
});
